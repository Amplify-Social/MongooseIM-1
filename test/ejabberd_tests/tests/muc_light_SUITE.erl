...
-spec check_process_memory_growing(pid(), integer(), integer()) -> any().
check_process_memory_growing(_, _, Counter) when Counter > 4 ->
    throw({memory_consumption_is_growing});
check_process_memory_growing(_, _, Counter) when Counter == 0 ->
    ok;
check_process_memory_growing(Pid, OldMemory, Counter) ->
  ...
  check_process_memory_growing(Pid, Memory, NewCounter).
